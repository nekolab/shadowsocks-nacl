...
int Socks5::ParseHeader(ConsultPacket &request,
  ...
  if (header[0] != VER || header[2] != RSV) { return 0; }
  ...
  switch (request.ATYP) {
    case IPv4:
      ...
      return 10;
    case IPv6:
      if (header.size() < 22) { return 0; }
      return 22;
    case DOMAINNAME:
      if (header.size() < (5 + header[4] + 2)) { return 0; }
      return 5 + header[4] + 2;
  }
...
int Socks5::PackResponse(std::vector<uint8_t> &resp,
  ...
  switch (reply.ATYP) {
    case IPv4: {
      ...
      for (uint8_t c : ipv4_addr.addr) { resp.push_back(c); }
    ...
    case IPv6: {
      ...
      for (uint8_t c : ipv6_addr.addr) { resp.push_back(c); }
