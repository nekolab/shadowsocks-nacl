...
void Encryptor::Decrypt(std::vector<uint8_t> &plaintext,
  ...
  if (dec_crypto_ == nullptr) {
    if (ciphertext.size() < cipher_info_->iv_size) { return; }

    dec_iv_.insert(dec_iv_.begin(), ciphertext.begin(), ciphertext.begin() + cipher_info_->iv_size);
    std::vector<uint8_t> payload(ciphertext.begin() + cipher_info_->iv_size, ciphertext.end());
...
void Encryptor::UpdateAll(const std::string password,
  ...
  if (enc == Crypto::OpCode::ENCRYPTION) {
    ...
    if (in.size() < info->iv_size) { return; }
